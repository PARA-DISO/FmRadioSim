...
#include <stdio.h>
// DEBUG CODE
#define ptr_print(in, out)                                                     \
  printf("C-Func::%s: read @ 0x%p / write @ 0x%p\n", __func__, in, out);       \
  fflush(stdout);
...
void _mm256_fprint_pd(FILE *fd, f64x4 v) {
  fprintf(fd, "%f\n%f\n%f\n%f\n", v.m256d_f64[0], v.m256d_f64[1],
          v.m256d_f64[2], v.m256d_f64[3]);
}
...
void print_filter_info(FilteringInfo *info) {
  ...
  printf("BPF-Info {\n"
         ...
         "  stage_hi: [%g, %g]\n"
         "}\n",
         info->prev_sig[0], info->prev_sig[1], info->prev_prev_sig[0],
         info->prev_prev_sig[1], info->prev_out[0], info->prev_out[1],
         info->prev_prev_out[0], info->prev_prev_out[1], info->stage[0],
         info->stage[1], info->stage[2], info->stage[3]);
  fflush(stdout);
}
void print_demodulate_info(DemodulationInfo *info) {
  fflush(stdout);
  printf("DemodulationInfo{\n"
         "  angle: [%g, %g, %g, %g]\n"
         "  prev_sin: [%g, %g, %g, %g]\n"
         "  prev_sig_lo: [%g, %g, %g, %g]\n"
         "  prev_sig_hi: [%g, %g, %g, %g]\n"
         ...
         info->prev_sin[0], info->prev_sin[1], info->prev_sin[2],
         info->prev_sin[3], info->prev_sig[0], info->prev_sig[1],
         info->prev_sig[2], info->prev_sig[3], info->prev_sig[4],
         info->prev_sig[5], info->prev_sig[6], info->prev_sig[7],
         info->prev_internal[0], info->prev_internal[1], info->prev_internal[2],
         info->prev_internal[3], info->prev_internal[4], info->prev_internal[5],
         info->prev_internal[6], info->prev_internal[7], info->filter_info[0],
         info->filter_info[1], info->filter_info[2], info->filter_info[3],
         info->filter_info[4], info->filter_info[5], info->filter_info[6],
         info->filter_info[7], info->filter_info[8], info->filter_info[9],
         info->filter_info[10], info->filter_info[11], info->filter_info[12],
         info->filter_info[13], info->filter_info[14], info->filter_info[15]);
  fflush(stdout);
}
void print_resampler_info(ResamplerInfo *info) {
  printf("ResamplerInfo {\n"
         "  prev: %g\n"
         "  multiplier: %lld\n"
         "  input_len: %lld\n"
         "}\n",
         info->prev, info->multiplier, info->input_len);
}
//----------------------------
...
//   return buf;
// }
// inline void differential(f64 *dr, f64 *di, const f64 r, const f64 i, f64
// *prev,
...
void fm_modulate(f64 *restrict output_signal, const f64 *restrict input_signal,
  ...
  for (usize i = 0; i < buf_len; i+=8) {
    f64x4 s1_in = _mm256_load_pd(input_signal + i);
    f64x4 s2_in = _mm256_load_pd(input_signal + i + 4);
...
void fm_demodulate(f64 output_signal[], const f64 input_signal[],
  ...
  // ptr_print(input_signal,output_signal);
  // printf("demodulate-crc-before: %lu\n",crc32((char*) input_signal, buf_len *
  // 8)); fflush(stdout);
  // //
  ...
  // 8)); fflush(stdout); printf("demodulate-crc-write: %lu\n",crc32((char*)
  // output_signal, buf_len * 8)); fflush(stdout);
...
void upsample(f64 *restrict dst, f64 *restrict input,
              ResamplerInfo *restrict info) {
  usize len = info->input_len;
  f64 prev = info->prev;
  usize multiplier = info->multiplier;
  double coeff = (double) multiplier;
  f64x4 offset = _mm256_set1_pd(4);
  f64x4 m = _mm256_set1_pd(1. / multiplier);
  f64x4 idx_base = _mm256_set_pd(3, 2, 1, 0);
// printf("len: %ld / multiplier: %ld\n", len,multiplier);
#pragma unroll
  for (int i = 0; i < len; ++i) {
#if SEPARATE_MODULATE_INTEGRAL
    double current_val = input[i];
    double s = prev + current_val * coeff; // 変調時のパラメータの整合性を確保
    f64x4 a = _mm256_set1_pd(prev);
    f64x4 b = _mm256_set1_pd(s);
    prev = s;
#else
    f64x4 a = _mm256_set1_pd(prev);
    ...
    for (usize j = 0; j < multiplier; j += 4) {
      ...
      f64x4 t = _mm256_fmadd_pd(a, coeff2, _mm256_mul_pd(b, coeff1));
...
void downsample(f64 *restrict dst, f64 *restrict input,
                ResamplerInfo *restrict info) {
  usize len = info->input_len;
