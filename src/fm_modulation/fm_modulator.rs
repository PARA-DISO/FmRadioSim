...
impl FmModulator {
    ...
    pub fn process_to_buffer(&mut self, signal: &[f64], buffer: &mut [f64]) {
        ...
        unsafe {fm_sys::fm_modulate(
          buffer.as_mut_ptr(), signal.as_ptr(),&raw mut self.prev_sig,&raw mut self.integral, self.sample_period, &raw mut self.t,self.modulation_index,self.carrier_freq, buffer.len() as u64
        )};
