...
impl FmDeModulator {
    pub fn new() -> Self {
        Self {
            ...
            sig_buffer: Vec::new(),
            low_pass: Lpf::new(79_500_000. * 3., CUT_OFF, Lpf::Q),
        }
    }
    pub fn from(f: f64, sample_rate: f64) -> Self {
        Self {
            ...
            diff_buffer: [Vec::new(), Vec::new()],
            sig_buffer: Vec::new(),
            low_pass: Lpf::new(sample_rate as f32, CUT_OFF, Lpf::Q),
        }
    }
    pub fn demodulate(&mut self, signal: &[f32]) {
        ...
        for i in 0..signal.len() {
            let (d_re, d_im) = { (self.diff_buffer[0][i], self.diff_buffer[1][i]) };
    ...
    fn differential(&mut self) {
        ...
        for i in 0..v1.len() {
            // 実部
            self.diff_buffer[0][i] = (-if i == 0 { self.prev_sig[0] } else { v1[i - 1] } + v1[i])
                / self.sample_period as f32;
            // 虚部
            self.diff_buffer[1][i] = (-if i == 0 { self.prev_sig[1] } else { v2[i - 1] } + v2[i])
                / self.sample_period as f32;
        }
