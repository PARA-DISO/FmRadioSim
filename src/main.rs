...
fn main() {
  ...
  let samples = reader.into_samples::<i16>().map(|s| s.unwrap()).collect::<Vec<i16>>();
  let [mut l_samples,mut r_samples] = samples.chunks(2).fold([Vec::<f32>::new(),Vec::<f32>::new()], |mut acc,samples| {
  ...
  let mut fm_sim = FmRadioSim::from(
      44100,
      CHUNK_SIZE,
      79_500_000f64,
  );
  ...
  l_samples.chunks(CHUNK_SIZE).zip(r_samples.chunks(CHUNK_SIZE)).for_each(|(l,r)| {
